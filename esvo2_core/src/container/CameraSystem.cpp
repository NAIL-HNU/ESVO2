  ...
  std::vector<double> &vT_b_c)
{
  ...
  K_inv_ = P_.block<3, 3>(0, 0).inverse();
  T_b_c_ = Eigen::Matrix<double,3,4,Eigen::RowMajor>(vT_b_c.data());
...
void CameraSystem::loadCalibInfo(const std::string &cameraSystemDir, bool bPrintCalibInfo)
{
  ...
  vT_b_c = leftCamCalibInfo["T_b_c"]["data"].as< std::vector<double> >();

  cam_left_ptr_->setIntrinsicParameters(
    width, height,
    cameraNameLeft,
    distortion_model,
    vD_left, vK_left, vRectMat_left, vP_left, vT_b_c);
  cam_right_ptr_->setIntrinsicParameters(
    width, height,
    cameraNameRight,
    distortion_model,
    vD_right, vK_right, vRectMat_right, vP_right, vT_b_c);
