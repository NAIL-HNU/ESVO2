  ...
  double visualization_threshold2)
{
  ...
  switch(vmType)
  {
    case InvDepthMap:
    {
      for (auto it = depthMapPtr->begin(); it != depthMapPtr->end(); it++)
      {
        if (it->valid() && it->variance() < pow(visualization_threshold1, 2)
          ...
          DrawPoint(it->invDepth(), max_range, min_range, it->x(), img, 1);
      }
      break;
    }
    case StdVarMap:
    ...
    case InvDepthMap_rel:
    {
      // cv::Mat img_old = cv::imread("/home/bell/data/rel.tiff", cv::IMREAD_UNCHANGED);
      cv::Mat img_rel = cv::Mat(cv::Size(width, height), CV_32FC1, cv::Scalar(0));
      for (auto it = depthMapPtr->begin(); it != depthMapPtr->end(); it++)
      {
        if (it->valid() && it->variance() < pow(visualization_threshold1, 2)
            && it->age() >= (int) visualization_threshold2)
            img_rel.at<float>(it->x()[1],it->x()[0]) = 1/it->invDepth();
      }
      break;
    }
    case InvDepthMap_rel2:
    {
      cv::Mat img_rel = cv::Mat(cv::Size(width, height), CV_32FC1, cv::Scalar(0));
      for (auto it = depthMapPtr->begin(); it != depthMapPtr->end(); it++)
      {
  ...
  cv::Mat &img )
{
  //define the color based on the inverse depth
  // int index = floor((val - min_range) / (max_range - min_range) * 255.0f);
  int index = 255 - floor((1/val - 0.7) / (50 - 0.7) * 255.0f);
  if (index > 255)
    index = 255;
  if (index < 0)
    index = 0;
  ...
  size_t row, size_t col)
{
  ...
  for(int i = 0; i < vEventPtr.size(); i++)
  {
    if(vEventPtr[i]->x >= 0 && vEventPtr[i]->x < col && vEventPtr[i]->y >= 0 && vEventPtr[i]->y < row)
